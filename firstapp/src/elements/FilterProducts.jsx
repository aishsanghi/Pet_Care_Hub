...
const FilterProducts = ({ loggeduser }) => {
  ...
  const getproductByCatId = async () => {
    try {
      const response = await axios.get(`${URL}/getproByCatId/${catId}`);
  ...
  const handleadd = async (item) => {
    ...
    try {
      ...
      const isInCart = pro.some((cartItem) => cartItem.product === item._id);
  ...
  return (
    <div className="expertbox">
      <div className="headingbox">
        <h2>All Products in this Category</h2>
      </div>
